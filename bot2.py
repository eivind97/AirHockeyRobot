        ...
        while started:
            ...
            if puck_pos != -1:
                if current_time != puck_pos_vel_points[-1][1]: # if time is not equal the last time value in table
                    ...
                    j += 1
                    k += 1
                    AtoDpoint = 0 # position where robot choose defence instead of attack
                    ...
                    if len(puck_pos_vel_points) == 5: # if table consist of 5 item
                        ...
                        if 100 < collision_pos[0] < 600 and 100 < collision_pos[1] < 600 and last_velocity[0] < 0 and collision_pos != -1:
                            _, current_time, puck_pos = self.camera.get_puck_coordinates() #Turn into a thread later
                            if puck_pos[0] < collision_pos[0] + 150:
                            ...
                            if collision_y != -1 and collision_y < 630 and collision_y > 70:  # if a point is found, and it is 100mm from wall
                                self.serialCom.writeData(Commands.MOVE_TO, pusher_bottomPos + 40, round(collision_y, 2), 3000)  # send move_to command, x-pos, y-pos and speed
                                self.last_botPos = Vec2d(pusher_bottomPos + 40, round(collision_y, 2))
