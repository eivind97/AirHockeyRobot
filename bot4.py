                    ...
                    if len(puck_pos_vel_points) == 5: # if table consist of 1 item
                        ...
                        if self.time_difference < 1.0: # if puck reach collision point before robot or 1 sec or more after
                            ...
                            if points[-2][0] < 300: # change to defence/attack mode later
                                ...
                                if 100 < collision_pos[0] < 600 and 70 < collision_pos[1] < 630 and last_velocity[0] < 0:
                                   # while x_wait_pos == 0 or x_wait_pos > collision_pos[0] + 0.01*last_velocity[0]:
                                    #    _, current_time, puck_pos = self.serialCom.readData(Commands.GET_BOTPOS) Turn into a thread
                                     #   ii += 1
                                     #   if type(puck_pos) == Vec2d:
                                     #       x_wait_pos = puck_pos[0]
                                     #   else:
                                      #      x_wait_pos = 0
                                       # if ii >= 25:
                                        #   break
                                    time.sleep(0.9*self.time_difference)
    ...
    def attack(self, puck_positions, bot_pos, times, puck_vel):
        ...
        if velocity[0] < 0:
            if len(puck_positions) >= 2:
                ...
                puck_lastPos = Vec2d(puck_positions[-1][0], puck_positions[-1][1])
                _, current_time, puck_pos = self.serialCom.readData(Commands.GET_BOTPOS)
